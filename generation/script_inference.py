...
def T5Trainer(dataframe, source_text, target_text, model_params, output_dir="../outputs/t5_0608"):
    ...
    inf_dataset = dataframe
    display_df(inf_dataset.head(2))

    console.print(f"INFERENCE Dataset: {inf_dataset.shape}\n")
    inf_set = DataSet(
        inf_dataset,
        tokenizer,
    ...
    titles = inf_set.source_text

    inf_params = {
        "batch_size": model_params["INF_BATCH_SIZE"],
        "shuffle": False,
        "num_workers": 0,
    }

    inf_loader = DataLoader(inf_set, **inf_params)

    console.log(f"[Initiating Inference ON INFERENCE SET]...\n")
    for epoch in range(model_params["INF_EPOCHS"]):
        predictions, actuals = inference(epoch, tokenizer, model, device, inf_loader)
    ...
    console.print(
        f"""[Inference] Generation on Inference data saved @ {os.path.join(output_dir,'predictions.csv')}\n"""
    )
...
if __name__ == "__main__":
    ...
    path = "../data/wikihow/wiki_step_intermediate.csv"
    df = pd.read_csv(path)
